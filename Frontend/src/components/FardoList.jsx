...
const cargarFardos = useCallback(async () => {
  ...
  try {
    ...
    await Promise.all(
      fardosCargados.map(async (fardo) => {
        try {
          ...
          mapClasificaciones[fardo.codigo_fardo] = clasificadasResponse.totalPrendasClasificadas;
        } catch (error) {
    ...
    setPrendasClasificadasMap(mapClasificaciones);
  } catch (error) {
...
const confirmarEliminarFardo = async () => {
  try {
    ...
    closeEliminarModal();
    cargarFardos(); // Vuelve a cargar los datos
  } catch (error) {
        ...
        <tr key={fardo.id} className="hover:bg-gray-50 transition duration-200">
          ...
          <td className="py-2 px-4 border-b flex justify-between items-center space-x-4">
            ...
            <div className="flex space-x-2">
              <button
                onClick={() => imprimirCodigoBarras(fardo.codigo_barra_fardo, fardo.categoria?.nombre_categoria, fardo.codigo_fardo)}
      ...
      {detalleFardo ? (
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <p className="text-gray-900">{detalleFardo.proveedor?.nombre_proveedor || 'N/A'}</p>
            <p className="text-gray-700 font-medium">Categoría:</p>
            <p className="text-gray-900">{detalleFardo.categoria?.nombre_categoria || 'N/A'}</p>
          </div>
